...
const service = {
  order: async (formDetails) => {
    const getProducts = async () => {
      const products = [];
      await axios.get(`${MS_URL}/products/productsInCart`).then((res) => {
    ...
    const getAllProducts = async () => {
      const allProducts = [];
      await axios.get(`${MS_URL}/products`).then((res) => {
    ...
    const composeHtmlContent = (products, allProducts) => {
      ...
      products.forEach((product) => {
        productListHTML = productListHTML.concat(
          `<li>${product.number}x ${product.title}</li>`
        );
      });
      if (products.length > 0) {
        productListHTML = productListHTML.concat(`</ul>`);
    ...
    await send_email(
      USER_EMAIL,
      "andrei.guta@stud.ubbcluj.ro",
      "New Order - Shopping APP",
      htmlContent
    );
    return 200;
  },
